...
/**
 * <b>Problem</b> class where all the characteristics of the input CNOP problem
 * object are defined.
 *
...
public class Problem {
    ...
    private ArrayList<double[]> mFinal;
...
//    /**
//     * Method that adds the  results found.
//     *
//     * @param obj matrix value
//     */
//    protected void addmResults(double[] obj) {
//        int sizeVector = this.getNumberVariableOF();
//
//        double[] aux = new double[sizeVector + 2];
//
...
//
//    /**
//     * Method that returns the results found.
//     *
//     * @return matrix value
//     */
//    public double[][] getmResults() {
//        double[][] resultsArrays = new double[this.mFinal.size()][];
//        for (int i = 0; i < resultsArrays.length; i++) {
//            resultsArrays[i] = this.mFinal.get(i);
//        }
...
//    }
     ...
     *
     * all elements within the iteration are separated by commas (,) and ranges
     * by colons (:) and outside the iteration are separated by semicolons (;).
     *
    ...
    /**
     * Method that receives the restrictions of the CNOP, these can be of two
     * types, equality or inequality. The correct syntax to include them is the
     * following: <br><br>
     *
     * [opening brace][mathematical expression][conditional
     * operator][results][closing brace][semicolon]<br><br>
     *
     * e.g:<br>
     * {-127 + 2 * x1^2 + 3 * x2^4 + x3 + 4 * x4^2 + 5 * x5 <= 0}<br><br>
     *
     * if there are more than one restriction they can be separated by
     * semicolons [;]<br>
     * {y1 + x1 <= 1.2}; {y2 + x2 <= 1.8}; {y2^2 + x2^2 <= 1.64}; ...;
     * [n-constraints] <br>
     *
    ...
    /**
     * Method that returns the number of independent executions of the
     * algorithm.
     *
    ...
    /**
     * Method that returns the time in seconds in which the configured execution
     * finished.
     *
    ...
    /**
     * Method that sets the time in seconds in which the configured execution is
     * finished.
     *
    ...
    /**
     * Method that returns data to plot the best results of the mean number of
     * runs.
     *
    ...
    public double[][] agregarColumna(double[][] matriz, double[] nuevaColumna) {
        ...
        for (int i = 0; i < filas; i++) {
            ...
            nuevaMatriz[i][columnas - 1] = nuevaColumna[i];
        }

        return nuevaMatriz;
    }

    public double[][] agregarFilas(double[][] matriz, double[] nuevaFila) {
        List<double[]> listaFilas = new ArrayList<>(Arrays.asList(matriz));
        listaFilas.add(nuevaFila);
        return listaFilas.toArray(new double[0][]);
    }

    /**
     * Method that returns a matrix; the number of columns represents the number
     * of CNOP variables to be solved. The number of rows represents the number
     * of assigned Sb bacteria, which is a matrix of all the solutions for that
     * CNOP.
     *
     * @return the Matix
     */
    public double[][] getMatix() {
        return this.Matix;
    }

    /**
     * With this method the CNOP solution matrix can assigned. Commonly used to
     * replace a solution space by a more optimal one.
     *
     * @param Matix the bacteriumMatix to set
     */
    public void setMatix(double[][] Matix) {
        this.Matix = Matix;
    }
}
